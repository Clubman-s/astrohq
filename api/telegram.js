...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (existingProfile) {
      // Проверяем, все ли данные уже заполнены
      if (existingProfile.birthdate && existingProfile.birthtime && existingProfile.city) {
        // Профиль завершен, запрашиваем тему прогноза
        ...
        // Если данные неполные, продолжаем запрашивать недостающую информацию
        let reply = `Здравствуйте! Вы уже начали заполнение данных. Пожалуйста, уточните:`;

        if (!existingProfile.birthtime) {
