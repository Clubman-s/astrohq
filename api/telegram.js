...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const userProfile = {
      session_id: chatId,
      birthdate: '1970-12-18', // Здесь формат даты должен быть YYYY-MM-DD
      birthtime: '07:00', // Пример времени рождения
      city: 'Москва',
    };

    const insertProfile = await supabase.from('user_profiles').upsert([userProfile]);

    if (insertProfile.error) {
      console.error('❗ Ошибка при сохранении профиля:', insertProfile.error);
    } else {
      console.log('📝 Профиль успешно сохранён:', insertProfile);
    }
